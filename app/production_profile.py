...
# Funding status describes KMÍ's RECORD, never asserts a film was unfunded (our ledger is 2021–24 only).
XREF_IS = {"matched": "Með úthlutun",
           "no_record_in_window": "Engin úthlutun skráð (innan tímabils)",
           "before_window": "Fyrir gagnatímabil KMÍ (2021–) — óþekkt",
           # legacy values (pre-rebuild DBs) mapped to the honest wording:
...
def _browse(st, db_path, qp):
    ...
    funded = c3.selectbox("KMÍ úthlutun", ["Allt", "Með úthlutun", "Engin úthlutun skráð"])
    ...
    if funded == "Með úthlutun":
        f = f[f["kmi_funded"] == 1]
    elif funded == "Engin úthlutun skráð":
        f = f[f["kmi_funded"] == 0]

    show = f[["title", "year", "kind", "director", "kmi_total_isk", "xref_status"]].copy()
    show["kind"] = show["kind"].map(lambda k: KIND_IS.get(k, k))
    show["director"] = show["director"].fillna("").map(lambda d: d if str(d).strip() else "óþekkt")
